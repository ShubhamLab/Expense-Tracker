...
import datetime
from pandas import Dateframe as df
import pandas as pd
...
def tracker(date = datetime.date.today(),income = 0 , expense = 0, desc = " "):
    data = { 'Date':date, 'desc':desc, 'Income':income, 'Expenses':expense }  
...
def Balance_get(date=datetime.date.today(),curr_month='May'):
    Bal = []
    ...
    for month in moonths[0:months.index(curr_month)+1]:
        try:
...
def total_balance_get():
    ...
    get = pd.read_csv("curr_month.csv",index_col='Date')
    get.dropna(inplace=True)
    Balance = (get['Income'].sum() - get['Expenses'].sum())
